...
def inspect_all_xgb_regressor_runs():
    ...
    try:
        # Filtrar ejecuciones cuyo nombre contenga 'xgb-sentiment' o 'xgb'
        runs = list(api.runs(project_path, filters={"display_name": {"$regex": "xgb-sentiment"}}))
    except Exception as e:
        print(f"❌ Error al conectar con W&B: {e}")
        return

    if not runs:
        print("⚠️ No se encontraron ejecuciones coincidentes con 'xgb-sentiment' en este proyecto.")
        return
    ...
    if not records:
        print("⚠️ Se encontraron runs de XGBoost Regressor, pero ninguna ha registrado las métricas `test_*` especificadas.")
        return

    df_metrics = pd.DataFrame(records)

    print(f"\n📊 EVOLUCIÓN HISTÓRICA DE MODELOS XGBOOST REGRESSOR ({len(df_metrics)} ejecuciones):")
    ...
    plt.title("Evolución de Métricas de Test entre Ejecuciones (XGBoost Regressor)", fontsize=13)
    ...
    output_plot = "xgb_regressor_test_evolution.png"
