...
def inspect_all_xgb_runs():
    ...
    print(f"🔗 Consultando historial completo de ejecuciones XGBoost en {project_path}...")
    
    try:
        # Filtrar ejecuciones cuyos nombres contengan 'xgb-classifier'
        runs = list(api.runs(project_path, filters={"display_name": {"$regex": "xgb-classifier"}}))
    except Exception as e:
        print(f"❌ Error al conectar con W&B: {e}")
        return

    if not runs:
        print("⚠️ No se encontraron ejecuciones de XGBoost en este proyecto.")
        return
    ...
    if not records:
        print("⚠️ Se encontraron runs de XGBoost, pero ninguna ha registrado métricas `cv_mean_*` aún.")
        return

    df_metrics = pd.DataFrame(records)

    print(f"\n📊 EVOLUCIÓN HISTÓRICA DE MODELOS XGBOOST ({len(df_metrics)} ejecuciones):")
    ...
    output_plot = "xgb_cv_evolution.png"
