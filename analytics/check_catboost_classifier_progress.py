...
import pandas as pd
import matplotlib
# Fijar backend no interactivo para evitar el UserWarning en CLI/servidor
matplotlib.use('Agg')
...
def inspect_all_catboost_runs():
    ...
    try:
        # Recuperar todas las ejecuciones filtradas por 'catboost'
        runs = list(api.runs(project_path, filters={"display_name": {"$regex": "catboost"}}))
    except Exception as e:
        print(f"❌ Error al conectar con W&B: {e}")
        return

    if not runs:
        print("⚠️ No se encontraron ejecuciones de CatBoost en este proyecto.")
        return
    ...
    for r in runs:
        ...
        for m in TARGET_METRICS:
            # 1. Intentar extraer de r.summary
            val = summary.get(m)
            
            # 2. Si no está en summary, intentar extraer de r.history()
            if val is None:
    ...
    if not records:
        print("⚠️ Se encontraron runs de CatBoost, pero ninguna ha registrado métricas `cv_mean_*` aún.")
        return

    df_metrics = pd.DataFrame(records)

    print(f"\n📊 EVOLUCIÓN HISTÓRICA DE MODELOS CATBOOST ({len(df_metrics)} ejecuciones):")
    ...
    plt.title("Evolución de Estadísticos CV entre Ejecuciones (CatBoost)", fontsize=13)
    ...
    output_plot = "catboost_cv_evolution.png"
    plt.savefig(output_plot, dpi=300)
    print(f"\n🖼️ Gráfica de evolución guardada en: {output_plot}")

if __name__ == "__main__":
    inspect_all_catboost_runs()