...
def inspect_all_xgb_regressor_runs():
    ...
    try:
        # Filtrar ejecuciones cuyo nombre contenga 'xgb-wf-huber-time-'
        runs = list(api.runs(project_path, filters={"display_name": {"$regex": "xgb-wf-huber-time-"}}))
    except Exception as e:
        print(f"❌ Error al conectar con W&B: {e}")
        return

    if not runs:
        print("⚠️ No se encontraron ejecuciones coincidentes con 'xgb-wf-huber-time-' en este proyecto.")
        return
    ...
    print(f"\n📊 EVOLUCIÓN HISTÓRICA DE MODELOS XGBOOST WALKFORWARD HUBER TIME ({len(df_metrics)} ejecuciones):")
    ...
    plt.title("Evolución de Métricas de Test entre Ejecuciones (XGBoost Walkforward HUBER TIME)", fontsize=13)
    ...
    output_plot = "xgb_walkerforward_huber_time_test_evolution.png"
