...
def inspect_all_xgb_regressor_runs():
    ...
    try:
        # Filtrar ejecuciones cuyo nombre contenga 'xgb-walkforward'
        runs = list(api.runs(project_path, filters={"display_name": {"$regex": "xgb-walkforward-"}}))
    except Exception as e:
        print(f"❌ Error al conectar con W&B: {e}")
        return

    if not runs:
        print("⚠️ No se encontraron ejecuciones coincidentes con 'xgb-walkforward' en este proyecto.")
        return
    ...
    if not records:
        print("⚠️ Se encontraron runs de XGBoost walkforward, pero ninguna ha registrado las métricas `cv_*` especificadas.")
        return
    ...
    plt.title("Evolución de Métricas de Test entre Ejecuciones (XGBoost Walkforward)", fontsize=13)
    ...
    output_plot = "xgb_walkerforward_test_evolution.png"
