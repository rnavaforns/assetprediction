...
# Walk-Forward CV + Embargo + Pseudo-Huber + Optuna

CONFIG = {
    "model_type": "XGBRegressor_WalkForward_Huber",
    "n_splits": 5,
    "embargo_days": 5,
    "optuna_trials": 15, # Ligeramente reducido ya que entrena n_splits por cada trial
    "random_state": 42,
...
def main():
    wandb.init(
        ...
        name=f"xgb-wf-huber-{datetime.now().strftime('%Y-%m-%d')}",
        group="ablation_study",
        tags=["walk-forward", "huber", "optuna", "final"],
        config=CONFIG
    )
    ...
    print(f"\n==================================================")
    print(f"Rendimiento Promedio CV (Pseudo-Huber) -> MAE: {avg_mae:.4f} | RMSE: {avg_rmse:.4f} | R2: {avg_r2:.4f}")
    ...
    model_path = "xgb_walkforward_huber_model.json"
    final_model.save_model(model_path)
    artifact = wandb.Artifact("xgboost-walkforward-huber", type="model")
